...
@section('contenu')
    ...
    <form action="{{ route('storeJeu') }}" method="POST" enctype="multipart/form-data" autocomplete="off">
        ...
        <a onclick="addEquipe()"><i class="fa fa-plus-circle" style="font-size:36px;color:     #007bff"></i></a>
    ...
    <script>
        ...
        function addEquipe() {
            $("#listeJoueurs").append('<div class="card border-primary mb-3 ml-3 mr-3 mt-3">' +
                '<div class="card-body" id="body">' +
                '<h5 class="card-title">@lang('contents.equipe') '+idE+'<input type="text" class="form-control col-1 float-sm-right"></h5>' +
                '<span id="content_'+idE+'">' +
                '<p>@lang('contents.joueurs')</p>' +
                '</span>' +
                '<a onclick="addJoueur('+idE+')"><i class="fa fa-plus-circle" style="font-size:36px;color:#007bff"></i></a>' +
                '</div>' +
                '</div>');
            addJoueur(idE);
            idE++;
